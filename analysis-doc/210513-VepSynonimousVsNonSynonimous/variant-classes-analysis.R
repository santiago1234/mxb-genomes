...
load_cat <- function(filepath) {
  ...
  cat <- basename(filepath) %>% 
    str_replace_all("(sfs-|.csv|-folded)", "")
...
all_cats <- 
  ...
  purrr::discard(.p = function(x) str_detect(x, "folded")) %>% 
  map_df(load_cat)
...
# Now redo-the plots with the folded spectrum
# ******** ----------------------------------------------------------------
# folded plots ----------------------------------------------------------
# ******** ----------------------------------------------------------------
# ******** ----------------------------------------------------------------
...
all_cats <- 
  list.files("results", pattern = "sfs-", full.names = T) %>% 
  purrr::keep(.p = function(x) str_detect(x, "folded")) %>% 
  map_df(load_cat)

# Drop the corners, in folded case the 0 (non-variants)
all_cats <- 
  all_cats %>% 
  filter(n != min(n), n != max(n))
...
all_cats <- 
  ...
  group_by(Population, Variant_Category) %>% 
  mutate(
    spectrum_norm = Minor_alle_freq / sum(Minor_alle_freq)
  ) %>% 
  ungroup()
...
# The SFS for populations and Variant Categories
general %>% 
  ggplot(aes(x = n, y = Minor_alle_freq)) +
  geom_bar(stat = "identity", position = "dodge") +
  facet_grid(Variant_Category ~ Population, scale = "free_y") +
  scale_y_sqrt() +
  labs(
...
ggsave("plots/SFS-all-folded.pdf", height = 7, width = 8)
ggsave("plots/SFS-all-folded.png", height = 7, width = 8)
...
ggsave("plots/pairwise-comparison-Categories-folded.pdf", height = 8, width = 10)
ggsave("plots/pairwise-comparison-Categories-folded.png", height = 8, width = 10)
...
ggsave("plots/SynVsIntergenic-folded.pdf", height = 4, width = 5)
ggsave("plots/SynVsIntergenic-folded.png", height = 4, width = 5)
